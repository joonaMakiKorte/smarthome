...
class HourlyWeather(BaseModel):
    ...
    model_config = ConfigDict(populate_by_name=True) # Enable alias usage in model initialization
    timestamp: int = Field(..., ge=0, description="Hour of the day (0-23) or UNIX timestamp")
    temperature: float = Field(..., description="Temperature in Celsius")
    icon_code: str = Field(..., description="OWM icon code", alias="iconCode")
    icon_url: str = Field(..., description="Full URL to the weather icon image", alias="iconUrl")
...
class CurrentWeather(BaseModel):
    ...
    temperature_feels_like: float = Field(...,
                                          ...
                                          alias="temperatureFeelsLike")
    humidity: int = Field(..., ge=0, le=100, description="Humidity percentage")
    wind_speed: float = Field(..., ge=0, description="Wind speed in m/s", alias="windSpeed")
